...
export default async function LeftHero() {
    ...
    return(
        <section className="flex flex-col gap-y-4 items-start justify-center">
            <HeroItemContainer title={t('contact')}>
                <ul className="flex flex-col gap-y-2">
                    <HeroContactListItem Icon={<LocationIcon/>} content="Santa Marta, Colombia"/>
                    <HeroContactListItem Icon={<MailIcon/>} link="mailto:wk.juanzuniga@gmail.com"
                                         content="wk.juanzuniga@gmail.com"/>
                    <HeroContactListItem Icon={<GitIcon/>} link="https://github.com/JuanZuniga79"
                                         content="JuanZuniga79" />
                    <HeroContactListItem Icon={<LinkedinIcon/>} link="https://www.linkedin.com/in/juanzuniga79/"
                                         content="/in/JuanZuniga79" />
                </ul>
            </HeroItemContainer>
            <HeroItemContainer title={t('info')}>
                <ul className="flex items-center justify-around flex-wrap pt-2 gap-y-5">
                    {professionalInfo.map((item, i) => (
                        <li key={i} className="flex flex-col items-center">
                            <p className="text-lg font-sans">
                                {item.key !== 'bc' && "+"}{item.value} {(item.key === 'experience' || item.key === 'programming') && t('years')}
                            </p>
                            <p className="font-mono font-bold text-base text-subtext-1 dark:text-dark-subtext-1">
                                {t(item.key)}
                            </p>
                        </li>
                    ))}

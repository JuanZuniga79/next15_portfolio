...
export default function HeroContactListItem({Icon, content, link}: Props) {
    return(
        <li className="flex items-center gap-x-2 text-text dark:text-dark-text">
            ...
            {link ? (
                <Link href={link} target="_blank" className="flex items-center gap-x-2 hover:underline">
                    {content} <LinkIcon/>
                </Link>
            ):(
                <span className="font-sans text-lg text-subtext-0 dark:text-dark-subtext-0">{content}</span>
            )}
        </li>
    )
}
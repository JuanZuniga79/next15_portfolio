...
import TimelineContainer, { Timeline, TimelineItem } from "../components/Timeline";
                ...
                <TimelineContainer title={e('work')} icon={<WorkIcon/>}>
                    <Timeline>
                        {experience.map((item, index) => (
                            <TimelineItem key={index} item={item} icon={<PCIcon/>} listTitle={e('achievement')}>
                                {item.keyAchievements.map((item, i)=> (
                                    <li key={i} className="gap-x-4 flex items-baseline">
                                        <figure className="min-w-1 h-1 rounded-full bg-text
                                            dark:bg-dark-text"/>
                ...
                <TimelineContainer title={e('education')} icon={<WorkIcon/>}>
                        <Timeline>
                            {education.map((item, index) => (
                                <TimelineItem key={index} item={item} icon={<EducationIcon/>} type={item.type}/>
                            ))}
                        </Timeline>
                </TimelineContainer>
            </div>
